package OOP.EX04_WORKING_WITH_ABSTRACTION.E06_GreedyTimes;
...
public class Bag {
    ...
    public void addGems(String item, long quantity) {
        long totalGems = getTotalGems();
        if (this.gold != null && hasFreeCapacity(quantity) && totalGems + quantity <= this.gold) {
    ...
    public void addGold(long quantity) {
        if (hasFreeCapacity(quantity)) {
            this.gold =
                    this.gold == null ? quantity : this.gold + quantity;
            //this.gold += quantity;
            this.currentTotalQuantity += quantity;
        }
    }
    ...
    public void printContent() {
        ...
        if (!this.gems.isEmpty()) {
            ...
            this.gems.entrySet().stream()
                    ...
                    .forEach(entry -> sb.append(String.format("##%s - %d", entry.getKey(), entry.getValue())).append(System.lineSeparator()));
        }
